	...
	switch($page[0]) {
		...
		case 'riverdashboard':
			if (!$sac = get_entity($page[1]) OR $sac->getSubtype() != 'shared_access') {
				...
				$body = elgg_view('riverdashboard/shared_access', array('sac' => $sac));
			}

			page_draw(elgg_echo('riverdashboard'),$body);
...
function shared_access_write_acl_plugin_hook($hook, $entity_type, $returnvalue, $params) {
	...
	// only show SACs in non-group contexts
	if ($page_owner instanceof ElggGroup) {
		// remove all SACs the user owns.
		$options = array(
			'type' => 'object',
		...
		if ($sacs = elgg_get_entities($options)) {
			foreach ($sacs as $sac) {
				unset($returnvalue[$sac->acl_id]);
			}
		}
	} elseif ($user AND $sacs = elgg_get_entities_from_relationship($options)) {
		foreach ($sacs as $sac) {
